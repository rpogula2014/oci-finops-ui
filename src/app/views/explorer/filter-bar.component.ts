...
import { DEFAULT_HIERARCHY, DIMENSION_FILTER_KEYS, DIMENSION_LABELS, DimensionFilterKey, FiltersStore } from '../../core/filters-store';
    ...
    <section class="filter-bar card" aria-label="Cost filters">
      <div class="controls">
          ...
          <label
            ...
            <select (change)="setFilter(control.key, $event)" [attr.aria-label]="control.label">
              ...
              @for (option of control.options; track option) {
                <option [value]="option" [selected]="filters.filter(control.key)() === option">
                  {{ labelFor(option) }}
                </option>
              }
            </select>
        ...
        <label
          ...
          ><select (change)="setGrain($event)">
            @for (grain of grains; track grain) {
              <option [value]="grain" [selected]="grain === filters.granularity()">
                {{ grain }}
              </option>
            }
          </select></label
      ...
      <div class="hierarchy">
        ...
        <select (change)="addDimension($event)" aria-label="Add hierarchy dimension">
          <option value="">Add dimension…</option>
          @for (dimension of remainingDimensions(); track dimension) {
            <option [value]="dimension">{{ labels[dimension] }}</option>
          }
        </select>
        <button (click)="filters.setHierarchy(DEFAULT_HIERARCHY)">Reset hierarchy</button>
        <button (click)="clearFilters()" [disabled]="!canClearFilters()">Clear filters</button>
      </div>
    </section>
  `,
  styles: `
...
export class FilterBarComponent {
  ...
  protected readonly canClearFilters = computed(() => DIMENSION_FILTER_KEYS.some((key) => this.filters.filter(key)() !== null) || Boolean(this.filters.search()));
  ...
  constructor() {
    ...
    toObservable(key)
      .pipe(
        debounceTime(150),
        switchMap(({ query }) =>
          forkJoin(this.optionRequests(query)).pipe(
            map((responses) => {
  ...
  protected clearFilters(): void {
    ...
    this.filters.selectedPath.set(null);
  }
  protected searchValue(event: Event): string {
    return (event.target as HTMLInputElement).value;
  }
  protected datePart(value: string): string {
  ...
  protected setGrain(event: Event): void {
  ...
  protected remove(index: number): void {
    this.filters.setHierarchy(this.filters.hierarchy().filter((_, current) => current !== index));
  }
  ...
  private optionRequests(query: CostQuery) {
    ...
    return [...groups.values()].map(({ query: upstream, fields }) => this.api.filters(upstream).pipe(map(({ rows }) => ({ fields, row: rows[0] }))));
  }
}
